...
interactiveButtons.send = function(target, data) {
  ...
  request(
    ...
    (err, res, body) => {
      if (err) {
        return logger.error(
          `error receiving response to interactive-buttons ${target}`,
          err
        );
      }
      ...
      if (typeof body === "string") {
        try {
          ...
          logger.error(
            "could not parse interactive-buttons response as json",
            e
          );
        }
      }
      ...
      logger.debug(
        `received response to interactive-buttons request: ${target}`
      );
